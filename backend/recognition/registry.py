...
def registry_lock():
    QueueControl.objects.get_or_create(name='model_registry')
    return QueueControl.objects.select_for_update().get(name='model_registry')
...
def activate_model(model_id=None, actor=None):
    with transaction.atomic():
        registry_lock()
...
def public_status():
    ...
    try:
        ...
        path = safe_path(settings.RECOGNITION_MODEL_ROOT, snapshot['artifact'])
    ...
    return {'enabled': True, 'model_name': model.name, 'model_version': model.version,
            'labels': [{'id': item['id'], 'name': item['name']} for item in model.labels],
