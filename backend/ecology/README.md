...
截至 2026-09-21，本模块的静态导览、模拟观测查询、历史导入及批次保留/维护工具已纳入北京部署。首页真实天气使用独立 [weatherdata 模块](../weatherdata/README.md)，不会覆盖下文的模拟 API 契约，也不写入本模块的历史 `Observation`。
...
- 开始时间必须有时区并对齐整点；一次限 1～744 小时及最多 50000 个观测点。模拟历史已有保留策略及手动预览确认维护，未启用定时自动生成或定时模拟批次删除，应避免无限频繁重跑大窗口。入口与保护条件见 [模拟批次维护](MAINTENANCE.md)。
...
- 旧 `weather-alerts` 接口未接上游，也未伪造官方预警。所有响应显式说明模拟或未接入状态；首页已通过独立的 `weather-data/summary/` 获取真实预警，不能由旧接口状态推断平台尚未接入真实天气。
...
测试覆盖模拟重放/幂等/缺失/峰值、失败批次回滚、历史导入原子性与去重、字段/来源/区域关联、查询边界和草稿不可见。M2 新增完整窗口聚合、真实 50000/50001 条边界、零值/缺失/存疑、来源与批次隔离、公开计数和地图版本保护测试；专项历史记录见 [M2 核心展示验证记录](../../docs/verification/M2核心展示验证记录.md)。最新批次维护还覆盖保留策略、真实数据保护、预览过期/重放、权限、事务回滚和并发互斥。2026-09-21 完整后端 377 项已在本机及北京隔离 PostgreSQL 通过，实际管理数据库流程 48 项通过；相关功能已部署，验收未删除现有模拟观测。证据见 [真实天气与管理验证记录](../../docs/verification/真实天气与管理验证记录.md)。
