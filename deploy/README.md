...
2026-09-20 已将源码 `26fe544` 部署到北京 Debian 13.2 实例，通过 283 项后端回归、71 次核心 HTTP 和 14 次 M2/LLM 关闭检查；数据库、依赖、API、花卉和河道服务及清理 timer 已配置。同日后续快照为 `c6ea5f6`，微信弹窗补丁未引入后端变更。服务器目前仍为该日版本，Key 为空、LLM 关闭，详见 [北京版本更新记录](../docs/verification/北京LLM版本更新记录.md)。**2026-09-21 的三板块问答、Markdown、悬浮入口和底栏/协议调整仅完成本机更新，未部署服务器；最新本机回归为后端 308 项、前端 246 项。**最初首尔部署设想已改为按北京实测 2 核、约 2GB 内存配置；公网 HTTPS、真实微信登录、手机与完整 M4 维护验收仍未完成。
...
L01–L06 增加独立的 `llm` 应用，2026-09-21 本机版本继续增加三板块公开上下文与分区计账迁移，不需要额外 Python 依赖或 Redis。更新到当前代码时先运行 `scripts/manage.sh migrate`；不能只替换小程序页面而沿用旧接口/数据库。保持密钥只在后端环境中：
...
本机界面在生态与科普相关页面提供悬浮问答入口，后端按当前模块核验公开资料；识别解读只能关联本人记录，可选发送净化图片。回答通过固定版本的本地 Markdown 解析器转为受限 `rich-text` 节点，支持标题、强调、列表、引用、代码和表格，不执行原始 HTML 或加载外链图片，无需额外 npm 构建。五个底栏入口保留文字并增加极简图标，中央 AI 识别按钮为上凸圆形并预留安全区。
...
2026-09-20 首轮本机真实联通使用原生微信开发者工具 Stable 2.02.2608070、基础库 3.7.12 的小程序模拟器，文字/附图各调用一次，合计 3131 token；按当时所有会话共用规则，验收时剩余 3/5。这是历史结果，不代表当前分区额度或累计费用。Key 仅存于被 Git 忽略的 `backend/.env`，权限 `0600`，未复制到服务器。详情见 [真实联通记录](../docs/verification/DeepSeek真实联通验证记录.md)。

**2026-09-21 当前本机手测保护：三个板块各每日五个成功回合；全站每日 50 次尝试（由此前 10 次调整）、100000 token、单次输出 600 token、并发 1。**最新本机自动回归为后端 308 项、前端 246 项。本轮未更新服务器，不包含手机或真实微信登录验收。
