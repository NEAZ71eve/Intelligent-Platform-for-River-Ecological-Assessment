...
## 7. 后续工作路线：从 YOLO 检测结果到生态环境评估

当前已交付的河道评估链路为 **YOLOv8n 检测（15 细类/4 大类）→ RULE v1 规则引擎 → 生态等级（优/良/中/差，100 分制）**，其中等级由规则引擎（漂浮物数量/面积占比、水华/黑臭面积、排污口、岸坡问题的固定扣分表）推导。

**后续的核心工作是把已有的 YOLO 算法检测结果进一步转化为真正的"生态环境评估"**，路线如下：

1. **检测结果生态指标化（先行，无需新标注）**
   - 由每图检测框聚合出生态指标：漂浮物密度（个/m²）、类别构成（瓶/泡沫/塑料/金属占比）、水面覆盖度（检测框面积占比）、空间分布特征（聚集度/条带分布）等。
   - 提供结构化 JSON 输出（每图指标 + 置信度），作为评估模型的标准化特征输入。

2. **生态等级评估模型（需生态等级标注）**
   - 在检测指标之上，对河道照片开展生态等级标签试点（双人标注，等级=优/良/中/差），积累标注集后训练监督评估模型（或检测+评估多任务头），替代/校准 RULE v1 固定扣分表。
   - 基线数据：`inference/data/splits/`（unified-v2/v3/v4 + water-eval）与外部评估集（eval-v1/v2）。

3. **部署衔接**
   - 评估模型沿现有登记管线（`register_model` + manifest + ONNX CPU 推理）发布，或在 `RuleSet` 表中登记新版本规则定义（版本化，唯一 `is_active=True`）。
   - 后端 `assessment-jobs/` API 契约不变：检测框 + 规则/模型版本快照 + score/grade/causes，历史任务不回写。

4. **验收口径**
   - 冻结 test（unified-v2 划分）与水域 held-out（water-eval，174 图）双口径对比；外部场景（eval-v1/v2，43 图）作为泛化抽查。
   - 等级准确性以双人标注一致性（Kappa）与业务方抽样复核为准。

详细实验记录见 `../docs/HYHQ_河道检测模型迭代报告_v3_v4_v5.md` 与 `../docs/训练评估体系强化方案.md`。
