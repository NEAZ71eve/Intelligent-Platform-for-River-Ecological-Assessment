...
当前阶段：**M2 静态导览、智慧河湖与数据中心已完成本地开发和联调；保留五类花卉识别与河道图像观察。环境数据继续使用模拟来源，微信端与其余 M2/M4 项目分别验收。**
...
当前开发顺序：M1 快照 → M3 图像识别 → 朋友仓库河道功能整合 → 北京基线内部验证 → M2 核心展示本地开发。M1 快照为 `6375b0c`，北京基线为 `358e222`，远程仓库为 [gruTGU/HYHQ](https://github.com/gruTGU/HYHQ)。本轮 M2 不部署服务器。下一批按计划推进科普与路线、个人记录与隐私、管理统计与审计；真机视觉和交互单独保留待办。
...
当前 M2 本地版本通过 PostgreSQL 后端全量回归、Node 前端回归及正常/浑浊/缺失三场景真实 HTTP + wx mock 联调，数量和复现方法见 [M2 验证记录](docs/verification/M2核心展示验证记录.md)。北京基线的数据库、依赖、Nginx、服务和真实推理已通过内部测试；本轮未连接或更新服务器。微信开发者工具、真机、真实河道精度、30 分钟服务器负载和公开发布尚未验收。
