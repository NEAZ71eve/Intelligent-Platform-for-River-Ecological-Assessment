...
当前阶段：**M1 基础版已保存 GitHub 快照；M3 接入五类花卉图像识别，环境数据继续使用模拟来源。**
...
识别需要本机登记并启用模型制品。模型权重不提交 Git，没有模型时明确返回 `MODEL_NOT_CONFIGURED`。首版只覆盖雏菊类、蒲公英类、蔷薇属花卉、向日葵类、郁金香类；不区分具体物种或品种，高分也不能保证排除范围外图片。天气、空气与河湖数据均明确标记为模拟数据。
...
当前开发顺序：M1 快照 → M3 图像识别。M1 快照为 `6375b0c`，远程为私有仓库 [gruTGU/HYHQ](https://github.com/gruTGU/HYHQ)。M2 的完整可视化、静态底图和其他未验收项仍保留在任务清单，不自动视为完成。
...
M1 快照通过 PostgreSQL 上的 44 项后端测试、17 项前端测试及真实 HTTP + wx mock 联调。后续验证分别记录；微信开发者工具、真机、首尔部署和公开发布条件尚未验收，不能从本机接口通过推定。
