...
- 图片上传、私有缩略图鉴权下载、识别任务状态查询。**没有模型时显示明确失败，不输出假预测。**

这是 M1 的可联调界面。静态导览点位、历史趋势图、详细业务筛选与正式真机能力属于后续验收；装饰示意图无真实地理含义。当前不请求定位。所有页面业务数据均来自 API，后端不可用时显示错误，不自动注入本地假数据。
...
6. 启动后端识别任务工作进程后，上传可用 JPEG/PNG/WebP 图片。M1 应进入明确的 `MODEL_NOT_CONFIGURED` 失败状态；没有启动工作进程时可能保持排队。前端最多自动查询约 30 秒，之后提供手动刷新。
