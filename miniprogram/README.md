...
- 可选 DeepSeek 结果解读：花卉/河道结果主动进入、分别确认文字和附图范围、首问手动发送；当前账号全部会话共用每日 5 轮额度，显示北京时间重置和处理中轮次。我的 AI 解读可分页查看及删除会话。
...
入口为完成后的花卉识别、河道观察结果卡片，以及“我的 → 我的 AI 解读”。`pages/llm` 只接受已有任务或会话，不提供无绑定结果的通用聊天。

1. 进入结果解读页仅查询本人的原始结果和 `llm/status/`，不自动创建会话或发送问题。
2. 默认只分享文字。用户可另外选择附图；选择变更后需要重新同意。确认弹窗说明识别结果、公开科普摘录、问题、会话上下文及可选的处理后原图会交给 DeepSeek。
3. 建立会话后首问预填，用户仍需点击发送；首次解读与后续问题均按服务端规则占用账号每日额度。原模型候选、检测框、分数不会由 LLM 修改。
4. 服务端未配置或关闭时显示原因；不把 API key 放进小程序。没有可确认额度时禁用发送。界面使用服务端日期与 `reset_at`，明确显示 UTC+8，不用设备本地时区推算重置。
5. 问答异步查询每 2.5 秒一次，最多自动查询 24 次。用户可停止、恢复；停止自动查询或离开页面不取消已发往外部服务的请求。网络结果未确认时保留问题，相同问题重试复用本次 `request_id`，避免重复占额。
6. 原图过期不会替换成留存缩略图。附图建会话时原图不可用会明确失败，用户可关闭附图重新确认；已有会话原图后来不可用时显示后续仅文字，每轮依据 `used_image` 标注实际情况。
7. 离开页面停止轮询并清除界面私有内容；重回页面重新读取服务端状态。旧账号请求、确认弹窗和延迟轮询不能写入新账号的视图。删除源任务会清理对应 AI 会话，删除 AI 会话不删除原任务。

定向检验：

```sh
node --test tests/llm.test.js tests/structure.test.js
```

测试使用本地 mock，覆盖显式同意/取消、不自动首问、重复点击、请求幂等重试、每日额度和北京时间、图片过期、轮询暂停恢复、会话删除、跨账号与生命周期、完整分页及原结果入口。不会调用真实 DeepSeek。真机授权、长文本滚动和网络体验需另行验证。
