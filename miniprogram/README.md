...
生态页面以 M1 可联调骨架为基础，识别页面支持 M3 模型协议。静态导览点位、历史趋势图、详细业务筛选与正式真机能力属于后续验收；装饰示意图无真实地理含义。河道图像观察仅在用户主动点击后请求可选定位，拒绝定位仍可上传；不会自动收集位置。所有页面业务数据均来自 API，后端不可用时显示错误，不自动注入本地假数据。
