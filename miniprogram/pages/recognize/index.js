...
Page({
  ...
  async load() {
    ...
    try {
      ...
      this.setData({ jobs });
      if (this.data.task && !this.data.busy) {
      ...
      if (!this._destroyed && generation === this._loadGeneration && this.current(token)) this.authError(error);
  ...
  async submit() {
    ...
    if (!this.data.capabilityKnown || !this.data.capability.enabled) { this.setData({ error: '河道生态评估模型未启用或状态未知，请稍后刷新。' }); return; }
    const token = app().session.token();
