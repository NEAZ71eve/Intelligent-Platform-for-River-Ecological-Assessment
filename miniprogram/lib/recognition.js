...
function resultView(result) {
  ...
  return {
    ...
    heading: recognized ? '识别结果：' + candidates[0].name : '暂时无法确认',
    explanation: recognized ? '结果仅适用于当前模型覆盖范围，可结合候选类别与科普资料进一步核对。' : lowQuality ? '图片质量不足，本次未进行类别判断。请上传尺寸足够、主体清晰的照片。' : '这张照片不足以给出确定结果。请对准单个主体重新拍摄，或确认对象属于支持类别。',
    candidates,
